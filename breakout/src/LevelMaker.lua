...
function LevelMaker.createMap(level)
    ...
    local numRows = 1 + math.min(math.floor(level/2), 4)
    ...
    --number of bricks that require keys
    local numBlockedBricks = math.floor(level/3)
    --one in 5 chance of more blocked Bricks
    if math.random(1,5) == 1 then
        numBlockedBricks = numBlockedBricks + 1
    end

    -- in the event we didn't generate any bricks, try again
    if #bricks == 0 then
        return self.createMap(level)
    --ensure that we don't try to block more bricks than there are, avoiding an infinite loop
    elseif numBlockedBricks > #bricks then
        return self.createMap(level)
    else
        local blockedBricks = {}
        for x = 1, numBlockedBricks do

            --ensure that a new brick is being blocked
            ::tryagain::
            local brickNum = math.random(1, #bricks)
            if bricks[brickNum].color == 6 then
                goto tryagain
            end
            bricks[brickNum].color = 6
            bricks[brickNum].tier = 1
            table.insert(blockedBricks, brickNum)
        end

        return bricks, blockedBricks
    end
end